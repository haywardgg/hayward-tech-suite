...
class RegistryManager:
    ...
    def _define_tweaks(self) -> List[RegistryTweak]:
        ...
        All tweaks are now loaded from JSON configuration files.
        This method returns an empty list as a placeholder for backward compatibility.
        
        Returns:
            Empty list (tweaks are loaded from JSON via _load_tweaks_from_json)
        """
        # All tweaks are now loaded from config/registry_tweaks.json
        # This keeps the codebase clean and makes tweaks easier to maintain
        return []
