...
class SystemToolsTab:
    ...
    def _create_prerequisites_section(self, parent: ctk.CTkFrame, start_row: int) -> int:
        ...
        # Section title
        title_label = ctk.CTkLabel(
            prereq_frame, text="Prerequisites Check", font=ctk.CTkFont(size=14, weight="bold")
        )
        title_label.grid(row=0, column=0, columnspan=2, padx=10, pady=(10, 5), sticky="w")

        # Admin status
        admin_label = ctk.CTkLabel(
            prereq_frame, text="Administrator Privileges:", font=ctk.CTkFont(size=11)
        )
        admin_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")

        is_admin = AdminState.is_admin()
        admin_status = (
            "✓ Running as Administrator" if is_admin else "⚠️ Not running as Administrator"
        )
        admin_color = "green" if is_admin else "orange"

        self.admin_status_label = ctk.CTkLabel(
            prereq_frame, text=admin_status, font=ctk.CTkFont(size=11), text_color=admin_color
        )
        self.admin_status_label.grid(row=1, column=1, padx=10, pady=5, sticky="w")

        # Winget availability
        winget_label = ctk.CTkLabel(
            prereq_frame, text="Windows Package Manager (winget):", font=ctk.CTkFont(size=11)
        )
        winget_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")

        self.winget_status_label = ctk.CTkLabel(
            prereq_frame, text="Checking...", font=ctk.CTkFont(size=11), text_color="gray"
        )
    ...
    def _check_prerequisites(self) -> None:
        ...
        def task():
            try:
                ...
                self.parent.after(
                    0,
                    lambda: self.winget_status_label.configure(
                ...
                self.parent.after(
                    0,
                    lambda: self.winget_status_label.configure(
                        text="⚠️ Check failed", text_color="red"
                    ),
                )
    ...
    def _create_tools_section(self, parent: ctk.CTkFrame, start_row: int) -> int:
        ...
        # Section title
        title_label = ctk.CTkLabel(
            tools_frame, text="Available Tools", font=ctk.CTkFont(size=14, weight="bold")
        )
    ...
    def _create_category_section(
        self, parent: ctk.CTkFrame, row: int, category: ToolCategory
    ) -> None:
    ...
    def _create_tool_row(self, parent: ctk.CTkFrame, row: int, tool: SystemTool) -> None:
        ...
        name_label = ctk.CTkLabel(
            top_frame, text=tool.name, font=ctk.CTkFont(size=12, weight="bold")
        )
        ...
        if tool.requires_admin:
            admin_badge = ctk.CTkLabel(
                top_frame, text="[Requires Admin]", font=ctk.CTkFont(size=11), text_color="orange"
            )
        ...
        status_label = ctk.CTkLabel(
            top_frame, text="Checking...", font=ctk.CTkFont(size=11), text_color="gray"
        )
    ...
    def _create_terminal_section(self, parent: ctk.CTkFrame, start_row: int) -> int:
        ...
        # Section title
        title_label = ctk.CTkLabel(
            terminal_frame, text="Installation Output", font=ctk.CTkFont(size=14, weight="bold")
        )
        title_label.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")

        # Terminal textbox
        self.terminal_text = ctk.CTkTextbox(
            terminal_frame, height=250, font=ctk.CTkFont(family="Consolas", size=10), wrap="word"
        )
        ...
        def task():
            try:
                ...
                for tool in self.installer.get_all_tools():
                    ...
                    self.parent.after(
                        0,
                        lambda: self._update_tool_ui_state(
    ...
    def _install_single_tool(self, tool: SystemTool) -> None:
        ...
        if self.is_installing:
            messagebox.showwarning(
                "Installation In Progress", "Please wait for the current installation to complete."
            )
            return
        ...
        def task():
            try:
                ...
                if success:
                    ...
                    self.parent.after(
                        0, lambda: messagebox.showinfo("Installation Successful", message)
                    )
                    ...
                    self.parent.after(
                        0,
                        lambda: (
                            self.tool_status_labels[tool.id].configure(
                    ...
                    self.parent.after(
                        ...
                        lambda: messagebox.showerror(
                            "Installation Failed", f"Failed to install {tool.name}:\n\n{message}"
                        ),
                    )
    ...
    def _update_tool_ui_state(
        self, tool_id: str, status_text: str, status_color: str, button_text: str, button_state: str
    ) -> None:
        """
